...
const DashboardEditFeedbackPage = () => {
  const url = usePathname();
  const parts = url.split("/");
  const feedbackId = Number(parts[parts.length - 2]);
  ...
  useEffect(() => {
    // Fetch feedback details on mount
    fetchDetailFeedback();
  }, []);

  useEffect(() => {
    // Update searchStudent when feedback.student_name changes
    setSearchStudent(feedback.student_name || "");
  ...
  const fetchDetailFeedback = async () => {
    try {
      const response = await getAdminDetailFeedback(feedbackId);
  ...
  const handlePutFeedback = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await putAdminFeedback(
        feedbackId,
        feedback.course_id,
        feedback.student_id,
        feedback.rating,
      ...
      toast.success("Berhasil edit feedback");
      window.location.href = "./../";
    } catch (error: any) {
      toast.error(`Gagal edit feedback 😔: ${error.message}`);
    }
  };
  ...
  return (
    <>
      {/* title */}
      <div className="mb-4">
        <p className="font-semibold text-neutral2 text-base">Edit Data</p>
        <p className="text-neutral3 text-sm">Feedback</p>
      </div>
      {/* form */}
      <form onSubmit={handlePutFeedback} className="max-w-5xl">
        ...
        <div className="text-end">
          <Button text="Edit Feedback" type="submit" />
        </div>
      </form>
      <ToastContainer />
    </>
...
export default DashboardEditFeedbackPage;
