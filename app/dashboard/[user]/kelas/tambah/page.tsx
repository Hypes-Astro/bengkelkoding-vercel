...
const DashboardTambahKelasPage: React.FC = () => {
  ...
  const handleSelectDosen = (dosenId: number, dosenName: string) => {
    setSelectedDosenId(dosenId); // Mengubah ID dosen yang dipilih

    if (StatusForm) {
      // Jika dalam mode edit, perbarui formDataEdit
      setFormDataEdit((prevData) => ({
        ...prevData,
        lecture_id: dosenId,
      }));
    } else {
      // Jika dalam mode tambah, perbarui formData
    ...
    setSearchTerm(dosenName); // Mengubah nama dosen pada input pencarian
    setIsDropdownOpen(false); // Tutup dropdown setelah dipilih
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.lecture_id) {
      console.error("ID dosen tidak boleh kosong");
      toast.error("Pilih dosen sebelum menyimpan.");
      return;
    }

    try {
      if (StatusForm === "edit") {
        await updateClassroom(formDataEdit, parseInt(IdClassroom));
  ...
  useEffect(() => {
    if (StatusForm && IdClassroom) {
      findDetailClassroom(IdClassroom, role_user)
        .then((response) => {
