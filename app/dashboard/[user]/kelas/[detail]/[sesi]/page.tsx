...
import { postUpdateStatusAbsence } from "@/app/api/absence";
import { getRole } from "@/app/api/general";
import SkeletonSesiKelas from "@/app/component/skleton/SkeletonSesiKelas";
...
const DetailKelasPageSesi = () => {
  ...
  useEffect(() => {
    const fetchRole = async () => {
      try {
        ...
        if (responseRole?.data?.role) {
          setUser({ role: responseRole.data.role });
        } else {
          console.error("Role data is missing in the API response.");
          setError("Role data is missing.");
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (user.role) {
      // console.log("User role:", user.role); // Log hanya ketika user.role berubah
    }
  }, [user.role]);
  ...
  const handleSaveStatus = async (student: Student, isAttend: boolean) => {
    ...
    try {
      let updateData;
      updateData = await postManualPresence(
        Number(sesi),
        updatedStatus,
        isAttend,
        role_user
  ...
  const fetchDetailSession = useCallback(async () => {
    try {
      ...
      presenceData = await getDetailQrSession(parseInt(sesi), role_user);
      setdetailClassRoom(presenceData.data);
    } catch (error) {
  ...
  const generateRandomText = async () => {
    try {
      ...
      let presenceQr;
      presenceQr = await getGenerateQr(sesi, role_user);
      setQrText(presenceQr.data.presence);
    } catch (error) {
  ...
  return (
    <>
      <div className=" flex flex-col w-full ">
        ...
        <div className="list-mhs flex flex-col mt-5 ">
          <div className="flex justify-between flex-wrap-reverse lg:flex-row gap-2">
                      ...
                      >
                        ...
                        <div className="mt-4 flex justify-end items-center">
                          {listMhs.is_present ? (
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() => handleSaveStatus(listMhs, true)}
                            ...
                            <td className="px-6 py-4">
                              {listMhs.is_present ? (
                                <Button
                                  text="cancel"
                                  onClick={() =>
                                    handleSaveStatus(listMhs, false)
                                  }
                                  className="block bg-red-500 hover:bg-red-600 p-1 rounded-md fill-white "
                                />
                              ) : (
                                <div className="flex gap-1">
                                  <button
                                    onClick={() =>
                                      handleSaveStatus(listMhs, true)
                                    }
            ...
            <div className="right-izin w-full md:w-1/2 lg:w-[25%]">
              <div className="flex flex-col  flex-wrap space-y-4 md:space-y-0 p-3 border-2 border-gray-200 border-dashed">
